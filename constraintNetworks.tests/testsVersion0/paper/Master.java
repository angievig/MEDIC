...
public class Master {
	static String path= "C:/Users/Angela Villota/Documents/Tests/Grupo/Execution/Problema7/";
	private static CSP csp;
	static private XCSPPrologParser Xparser;
	static String filename= "Renault-master-pos.xml";
...
//		X7_75#=1,
//		X7_80#=1,
		cn1= new Constraint("CN1","X2_1#=1");
		csp.addConstaint(cn1);
		csp.addVarToConstraint(cn1, "X2_1");
		
		cn2= new Constraint("CN2","X4_23#=0");
		csp.addConstaint(cn2);
		csp.addVarToConstraint(cn2, "X4_23");

//		cn3= new Constraint("CN3","X1_1#=0");
//		csp.addConstaint(cn3);
//		csp.addVarToConstraint(cn3, "X1_1");
//
//		cn4= new Constraint("CN4","X4_5#=0");
//		csp.addConstaint(cn4);
//		csp.addVarToConstraint(cn4, "X4_5");
//
...
//		csp.addVarToConstraint(cn6, "X7_80");
	...
	static void tests(ConstraintGraph net){
		...
		for (int i = 0; i < 1; i++) {
			...
			MinimalSetsDFSNoLog minimal = new MinimalSetsDFSNoLog(csp,path+"out/", fman, net);
